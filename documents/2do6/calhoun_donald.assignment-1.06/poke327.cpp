...
int main(int argc, char *argv[])
{
  ...
  string fname = "pokemon_moves.csv";
  ...
  vector<Moves> moves;
  vector<Pokemon_Moves> pokemon_moves;
  vector<Species> species;
  vector<Experience> experience;
  vector<Type_Names> type_names;
 
	fstream file ("/share/cs327/" + fname, ios::in);
    if (!file.is_open()) {
        file.close();
        char* homepath = getenv("HOME");
        string home(homepath);
        file.open(home + "/.poke327/" + fname, ios::in);
    }
  ...
  int count = 0;
  int i = 1;
	for(i=1;(unsigned)i<content.size();i++) {
        if (fname == "pokemon.csv") {
            Pokemon pk;
            pk.id = atoi(content[i][0].c_str());
            pk.identifier = content[i][1];
            pk.species_id = atoi(content[i][2].c_str());
            pk.height = atoi(content[i][3].c_str());
            pk.weight = atoi(content[i][4].c_str());
            pk.base_experience = atoi(content[i][5].c_str());
            pk.order = atoi(content[i][6].c_str());
            pk.is_default = atoi(content[i][7].c_str());
            pokemons.push_back(pk);
            count = 7;
        } else if (fname == "moves.csv") {
            Moves mov;
            mov.id = atoi(content[i][0].c_str());
            mov.identifier = content[i][1];
            mov.generation_id = atoi(content[i][2].c_str());
            mov.type_id = atoi(content[i][3].c_str());
            mov.power = atoi(content[i][4].c_str());
            mov.pp = atoi(content[i][5].c_str());
            mov.accuracy = atoi(content[i][6].c_str());
            mov.priority = atoi(content[i][7].c_str());
            mov.target_id = atoi(content[i][8].c_str());
            mov.damage_class_id = atoi(content[i][9].c_str());
            mov.efect_id = atoi(content[i][10].c_str());
            mov.effect_chance = atoi(content[i][11].c_str());
            mov.contest_type_id= atoi(content[i][12].c_str());
            mov.contest_effect_id = atoi(content[i][13].c_str());
            if (content[i].size() < 15) {
                mov.super_contest_effect_id = -1;
            } 
            else {
                mov.super_contest_effect_id = atoi(content[i][14].c_str());
            }
            moves.push_back(mov);
            count = 14;
        } else if (fname == "pokemon_moves.csv") {
            Pokemon_Moves pk_mov;
            pk_mov.pokemon_id = atoi(content[i][0].c_str());
            pk_mov.version_group_id = atoi(content[i][1].c_str());
            pk_mov.move_id = atoi(content[i][2].c_str());
            pk_mov.pokemon_move_method_id = atoi(content[i][3].c_str());
            pk_mov.level= atoi(content[i][4].c_str());
            if (content[i].size() < 6) {
                pk_mov.order = -1;
            } 
            else {
                pk_mov.order = atoi(content[i][5].c_str());
            }
            pokemon_moves.push_back(pk_mov);
            count = 5;
        } else if (fname == "pokemon_species.csv") {
            Species spec;
            spec.id = atoi(content[i][0].c_str());
            spec.identifier = content[i][1];
            spec.generation_id = atoi(content[i][2].c_str());
            spec.evolves_from_species_id = atoi(content[i][3].c_str());
            spec.evolution_chain_id = atoi(content[i][4].c_str());
            spec.color_id = atoi(content[i][5].c_str());
            spec.shape_id = atoi(content[i][6].c_str());
            spec.habitat_id = atoi(content[i][7].c_str());
            spec.gender_rate = atoi(content[i][8].c_str());
            spec.capture_rate = atoi(content[i][9].c_str());
            spec.base_happiness = atoi(content[i][10].c_str());
            spec.is_baby = atoi(content[i][11].c_str());
            spec.hatch_counter= atoi(content[i][12].c_str());
            spec.has_gender_differences = atoi(content[i][13].c_str());
            spec.growth_rate_id = atoi(content[i][14].c_str());
            spec.forms_switchable= atoi(content[i][15].c_str());
            spec.is_legendary = atoi(content[i][16].c_str());
            spec.is_mythical= atoi(content[i][17].c_str());
            spec.order = atoi(content[i][18].c_str());
            if (content[i].size() < 20) {
                spec.conquest_order = -1;
            } 
            else {
                spec.conquest_order = atoi(content[i][19].c_str());
            }
            species.push_back(spec);
            count = 19;
        } else if (fname == "experience.csv") {
            Experience exp;
            exp.growth_rate_id = atoi(content[i][0].c_str());
            exp.level= atoi(content[i][1].c_str());
            if (content[i].size() < 3) {
                exp.experience= -1;
            } 
            else {
                exp.experience = atoi(content[i][2].c_str());
            }
            experience.push_back(exp);
            count = 2;
        } else if (fname == "type_names.csv") {
            Type_Names typ;
            typ.type_id = atoi(content[i][0].c_str());
            typ.local_language_id = atoi(content[i][1].c_str());
            if (content[i].size() < 3) {
                typ.name = -1;
            } 
            else {
                typ.name = atoi(content[i][2].c_str());
            }
            type_names.push_back(typ);
            count = 2;
        }
        ...
        if (content[i].size()-1 < (unsigned)count) {
            cout<<content[0][count]<<": \n";
        }
		cout<<"\n";
	}
